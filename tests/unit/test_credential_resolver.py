...
class TestParseEnvFile:
    ...
    def test_multiple_keys(self, tmp_path):
        path = _write_env_file(
            tmp_path,
            """\
            GH_AUDIT_TOKEN=ghp_test
            GH_AUDIT_ORGANIZATION=myorg
            """,
        )
        result = parse_env_file(path)
        assert result["GH_AUDIT_TOKEN"] == "ghp_test"
        assert result["GH_AUDIT_ORGANIZATION"] == "myorg"
...
class TestResolveSettingsAuthMethod:
    ...
    def test_pat_auth_method(self, monkeypatch):
        ...
        monkeypatch.delenv("GH_AUDIT_ORGANIZATION", raising=False)
...
class TestResolveSettingsHierarchy:
    ...
    def test_cli_overrides_env_token(self, monkeypatch):
        monkeypatch.setenv("GH_AUDIT_TOKEN", "ghp_from_env")
        monkeypatch.setenv("GH_AUDIT_ORGANIZATION", "env-org")
    ...
    def test_cli_overrides_env_organization(self, monkeypatch):
        monkeypatch.setenv("GH_AUDIT_ORGANIZATION", "env-org")
        monkeypatch.setenv("GH_AUDIT_TOKEN", "ghp_env")
    ...
    def test_dotenv_fallback_when_no_env_vars(self, monkeypatch, tmp_path):
        ...
        env_file = _write_env_file(
            tmp_path,
            """\
            GH_AUDIT_TOKEN=ghp_dotenv
            GH_AUDIT_ORGANIZATION=dotenv-org
            """,
        )
    ...
    def test_env_path_override(self, monkeypatch, tmp_path):
        ...
        custom_env.write_text("GH_AUDIT_TOKEN=ghp_custom\nGH_AUDIT_ORGANIZATION=custom-org\n")
...
class TestResolveSettingsTokenValidation:
    ...
    def test_non_ascii_token_from_env_raises_config_error(self, monkeypatch):
        monkeypatch.setenv("GH_AUDIT_TOKEN", "ghp_caf\u00e9")
...
class TestResolveSettingsOptionalFields:
    ...
    def test_api_url_from_cli(self, monkeypatch):
        monkeypatch.delenv("GH_AUDIT_API_URL", raising=False)
        settings = resolve_settings(
            token="ghp_test",
            organization="myorg",
    ...
    def test_api_url_from_env(self, monkeypatch):
        ...
        monkeypatch.setenv("GH_AUDIT_ORGANIZATION", "myorg")
        monkeypatch.setenv("GH_AUDIT_API_URL", "https://ghes.example.com/api/v3")
        settings = resolve_settings()
    ...
    def test_app_credentials_from_env(self, monkeypatch, tmp_path):
        ...
        key_file.write_text("fake-pem")
        monkeypatch.delenv("GH_AUDIT_TOKEN", raising=False)
        monkeypatch.setenv("GH_AUDIT_APP_ID", "42")
        monkeypatch.setenv("GH_AUDIT_PRIVATE_KEY_PATH", str(key_file))
        monkeypatch.setenv("GH_AUDIT_INSTALLATION_ID", "99")
        monkeypatch.setenv("GH_AUDIT_ORGANIZATION", "app-org")
        settings = resolve_settings()
        assert settings.auth_method == "github_app"
    ...
    def test_app_credentials_from_dotenv(self, monkeypatch, tmp_path):
        ...
        key_file.write_text("fake-pem")
        monkeypatch.delenv("GH_AUDIT_TOKEN", raising=False)
        monkeypatch.delenv("GH_AUDIT_APP_ID", raising=False)
        monkeypatch.delenv("GH_AUDIT_PRIVATE_KEY_PATH", raising=False)
        monkeypatch.delenv("GH_AUDIT_INSTALLATION_ID", raising=False)
        monkeypatch.delenv("GH_AUDIT_ORGANIZATION", raising=False)
        env_file = _write_env_file(
            tmp_path,
            f"""\
            GH_AUDIT_APP_ID=7
            GH_AUDIT_PRIVATE_KEY_PATH={key_file}
            GH_AUDIT_INSTALLATION_ID=55
            GH_AUDIT_ORGANIZATION=dotenv-app-org
            """,
        )
        settings = resolve_settings(env_path=env_file)
        assert settings.auth_method == "github_app"
